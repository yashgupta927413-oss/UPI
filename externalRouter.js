...
function createExternalRouter(dbPool) {
  ...
  router.post('/pay/initiate', authenticateApiKey, async (req, res) => {
    ...
    try {
      ...
      const upiUri = `upi://pay?pa=${encodeURIComponent(assignedUpi)}&pn=${encodeURIComponent(accountHolder)}&am=${finalAmount.toFixed(2)}&cu=INR&tn=${encodeURIComponent(orderId)}`;
